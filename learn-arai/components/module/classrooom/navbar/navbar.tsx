...
export default function Navbar() {
    ...
    const clickHandle = () => {
        window.addEventListener('click', function(e) {
            if ( !this.document.getElementById('drop-down')?.contains(e.target as Node)) {
                setIsMenuOpen(false);
            } else {
        ...
        <>
            <nav className="flex items-center shadow p-3 px-6">
                ...
                <div id='drop-down' className="drop-down mx-auto mr-0">
                    <button className="drop-down-btn hover:bg-muted mx-auto mr-0 p-3 rounded-full" onClick={ clickHandle }>
                        <FaPlus/>
                    </button>

                    <div className={`drop-down-menu ${ isMenuOpen ? 'block' : 'hidden'}`}>
                        <CreateClassroom/>
                        <br />
                        <JoinClassroom/>
                    </div>
